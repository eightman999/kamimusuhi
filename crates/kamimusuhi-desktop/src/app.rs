    ...
    fn connections_view(&self, ui: &mut egui::Ui) {
        ...
        if let Some(connection) = &self.connection {
            ...
            card(ui, "生成・選択のルール", |ui| {
                for note in [
                    "primaryを含め、有効・privacy許可・必要な認証が設定済みの全器官を同時に開始します。provider数にハード上限はありません。",
                    "最初の有効完了を10msの共通graceで受け、Jevへ送るactive poolは最大2候補です。ACCEPTなら遅い器官を待たずに返します。",
                    "Jevへはcandidate-Nの匿名IDだけを渡し、provider名・model名は見せません。空の応答と16 KiB超の応答を除外し、有効な応答の全文を評価します。",
                    "RETRY/REJECTなら次に完了した候補batchを再判定し、全候補を使ってもACCEPTがなければ最初のRETRY候補だけを1回修復します。",

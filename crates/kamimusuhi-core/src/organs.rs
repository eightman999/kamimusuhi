...
impl OrganSupervisor {
    ...
    pub fn run(&mut self, input: &OrganInput) -> OrganCycle {
        ...
        let mut active_with_deadlines = Vec::new();
        for organ in &mut self.organs {
            ...
            match result {
                Ok(signal) if signal.influences_cognition() => {
                    let deadline = completed_at
                        .checked_add(Duration::from_millis(signal.ttl_ms))
                        .unwrap_or(completed_at);
                    active_with_deadlines.push((deadline, signal));
                }
        ...
        cycle.active = active_with_deadlines
            .into_iter()
            .filter_map(|(deadline, signal)| (finished_at < deadline).then_some(signal))
            .collect();
        cycle
    }
}
...
mod tests {
    ...
    struct SlowFixtureOrgan {
        descriptor: OrganDescriptor,
        ttl_ms: u64,
        delay_ms: u64,
    }

    impl CognitiveOrgan for SlowFixtureOrgan {
        fn descriptor(&self) -> OrganDescriptor {
            self.descriptor.clone()
        }
        ...
        fn process(&mut self, input: &OrganInput) -> Result<OrganSignal, OrganError> {
            ...
            Ok(OrganSignal {
                ...
                ttl_ms: self.ttl_ms,
                confidence_milli: Some(900),
                payload: serde_json::json!({"delay_ms": self.delay_ms}),
                evidence_refs: input.evidence_refs.clone(),
            })
        }
    }
    ...
    #[test]
    fn supervisor_drops_active_signals_that_expire_during_later_organs() {
        let mut supervisor = OrganSupervisor::new();
        supervisor
            .register(Box::new(SlowFixtureOrgan {
                descriptor: descriptor(
                    "a-expiring",
                    ExperimentVerdict::Pass,
                    PromotionMode::Active,
        ...
        supervisor
            .register(Box::new(SlowFixtureOrgan {
                descriptor: descriptor("z-fresh", ExperimentVerdict::Pass, PromotionMode::Active),
                ttl_ms: 1_000,
                delay_ms: 25,
            }))
            .unwrap();
        ...
        assert_eq!(cycle.active.len(), 1);
        assert_eq!(cycle.active[0].descriptor.key.as_str(), "z-fresh");
    }

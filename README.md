...
選択は生成済み応答の品質を優先し、遅延・成功率は補助情報です。Jevへは `candidate-N` の匿名IDだけを渡し、provider名・model名は見せません。空の応答と16 KiB超の応答を除外し、有効な応答と実際の出典付き根拠を全文で評価します。RETRY/REJECTなら次の完了候補を判定し、全providerを使い切ってもACCEPTがなければ最初のRETRY候補だけを1回修復します。Jev自体が応答不能（timeout・通信・TLS・HTTP status・credential）なら、そのターンはrule-based gateへdegradeして会話を継続し、障害codeをtraceへ残します。形式不正・無効なchoice・provider申告のfallback・明示的なREJECT/WAITはこれまでどおりfail-closedで、自動ACCEPTやprimary選択で補いません。キー未設定時はMock互換のrule-based経路を使い、意味評価は未実施と明示します。実APIでの精度・速度改善は未検証です。

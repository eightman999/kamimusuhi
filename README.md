...
常駐個体（Pi）と話す既定モードは、macOSアプリとしても起動できます。
`~/.config/kamimusuhi/nodes` と `node_token` を読むので、Finderから起動しても同じ接続先を使います。

    # target/macos/Kamimusuhi.app を作り、~/Applications へ配置

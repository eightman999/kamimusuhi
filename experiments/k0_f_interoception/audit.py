...
def audit(root,primary,policy=None,live=None,llm=None):
    ...
    for kind,folder in collections.items():
        ...
        for frame,row in zip(frames,aligned):
            ...
            same_values=len(frame['values'])==len(expected['values']) and all(math.isclose(a,b,rel_tol=0,abs_tol=1e-12) for a,b in zip(frame['values'],expected['values']))
            if not same_values or any(frame[k]!=expected[k] for k in ['mask','quality','normalization_identity']):norm_errors.append(kind+': frame mismatch '+frame['frame_id'])

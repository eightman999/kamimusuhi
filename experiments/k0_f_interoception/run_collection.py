...
def main():
    p=argparse.ArgumentParser();p.add_argument('--ssh-alias',required=True);p.add_argument('--peer-host',required=True);p.add_argument('--remote-root',required=True);p.add_argument('--remote-python',required=True);p.add_argument('--sensor-binary',type=Path,required=True);p.add_argument('--output',type=Path,required=True);p.add_argument('--remote-output',required=True);p.add_argument('--blocks',type=int,default=48);p.add_argument('--block-seconds',type=float,default=20);p.add_argument('--source-commit',required=True);p.add_argument('--phase',choices=['record','live','llm'],default='record');p.add_argument('--training-artifacts');p.add_argument('--dataset');p.add_argument('--model-python');p.add_argument('--model-loader')
    a=p.parse_args()
    module={'live':'experiments.k0_f_interoception.live','record':'experiments.k0_f_interoception.acquire','llm':'experiments.k0_f_interoception.record_llm'}[a.phase]
    extra=[]
    if a.phase=='live':
        if not a.training_artifacts or not a.dataset:raise ValueError('live phase requires frozen models and dataset')
        extra=['--training-artifacts',a.training_artifacts,'--dataset',a.dataset]
    if a.phase=='llm':
        if not a.model_python or not a.model_loader:raise ValueError('LLM recording requires existing local loader')
        extra=['--model-python',a.model_python,'--model-loader',a.model_loader]
    ...
    def transfer(seconds,block_stop):
        end=time.monotonic()+seconds;count=0;payload=bytes(4*1024*1024)
        while time.monotonic()<end and not network_done.is_set() and not block_stop.is_set():
            started=time.time()
    ...
    try:
        ...
        driver=subprocess.Popen(['ssh',a.ssh_alias,remote([a.remote_python,'-m',module,'collect','--output',a.remote_output,'--blocks',a.blocks,'--block-seconds',a.block_seconds,'--source-commit',a.source_commit]+extra)],stdout=subprocess.PIPE,stderr=subprocess.STDOUT,text=True);processes.append(driver)
        ...
        for line in driver.stdout:
            ...
            if event.get('label')=='network_transfer':
                active_network_stop=threading.Event()
                t=threading.Thread(target=transfer,args=(event['seconds'],active_network_stop),daemon=True);t.start();threads.append(t)
            if event.get('event')=='block_end':active_network_stop.set()
        ...
        try:
            if driver and driver.poll() is None:
                ...
                try:driver.wait(timeout=30)
                    ...
                    force=remote([a.remote_python,'-c',"import json,os,signal,sys; from pathlib import Path; d=json.loads(Path(sys.argv[1]).read_text()); p=d['pid']; c=Path('/proc/'+str(p)+'/cmdline').read_bytes(); assert d['owned_by']=='k0-f-acquire' and (b'experiments.k0_f_interoception.acquire' in c or b'experiments.k0_f_interoception.live' in c or b'experiments.k0_f_interoception.record_llm' in c); os.kill(p,signal.SIGTERM)",a.remote_output+'/driver_identity.json'])
                    subprocess.run(['ssh','-o','ConnectTimeout=5',a.ssh_alias,force],capture_output=True,timeout=10)
                    driver.wait(timeout=15)
        except BaseException as exc:
            cleanup_errors.append({"stage":"remote_stop","error_type":type(exc).__name__})
        network_done.set()
        ...
        try:
            ...
            cleanup_errors.append({'stage':'remote_verification','error_type':type(exc).__name__})
        (a.output/'transport_state.json').write_text(json.dumps(dict(timestamp=time.time(),success=success and not cleanup_errors,cleanup_errors=cleanup_errors,remote_cleanup_state=remote_state,owned_processes=[dict(pid=proc.pid,returncode=proc.poll()) for proc in processes],sensor_source_retained=True,connection_values_excluded=True),indent=2)+'\n')

...
def exact_sign_test(differences, tolerance: float = 1e-12) -> float:
    ...
    if values.ndim != 1 or not np.isfinite(values).all():
        raise ValueError("sign-test differences must be a finite vector")
...
def describe(values, *, bootstrap_seed: int = 1847, resamples: int = 20000) -> dict:
    ...
    return {"n_seeds": len(x), "mean": math.fsum(x.tolist()) / len(x),
...
def paired_comparison(left, right) -> dict:
    ...
    result["pass"] = bool(len(a) == 12 and result["mean"] >= .01 and
                          result["ci95"][0] > 0 and result["exact_sign_p"] <= .05)
    result["minimum_effect"] = .01
    result["required_seeds"] = 12
    result["multiplicity"] = "intersection-union: P1-P4 all required; diagnostic rows cannot replace a failed endpoint"
    return result
